...
- 新闻分析面向专业投资者，不得只复述标题。`summary_zh` 用一句话说明已确认事实；`rationale_zh` 用不超过三句说明受影响的财务、预期、估值或风险变量、主要催化剂及判断失效条件。分析可以更深，但不得增加背景铺陈、泛泛风险提示或“投资者需密切关注”等空话；输入证据不足时明确证据边界，不得补造事实。
