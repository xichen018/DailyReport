    ...
    def test_complete_run_writes_all_artifacts_and_isolated_requests(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for call in client.calls:
                ...
                self.assertIn("`provider_data.market.fundamentals` 是 SEC Company Facts 一级来源", call["prompt"]["common_rules"])
                ...
                if call["task_id"] == "macro_market":
                    ...
                    self.assertIn("会议日历不等于声明发布时间", call["prompt"]["module_instructions"])
                if call["task_id"] == "cross_asset":
